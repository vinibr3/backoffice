...
RSpec.describe WithdrawParametrization, type: :model do
  let(:withdraw_parametrization) { build(:withdraw_parametrization) }
  ...
  context 'when assign minimum_amount' do
    let(:withdraw_parametrization) { build(:withdraw_parametrization, minimum_amount: amount) }

    it 'saves as multiple of 1e8' do
      expect(withdraw_parametrization[:minimum_amount]).to eq((amount.to_f * 1e8).to_i)
    end

    it 'return as decimal of 1e8' do
      expect(withdraw_parametrization.minimum_amount).to eq(amount)
    end
  end

  context 'when assign maximum_amount' do
    let(:withdraw_parametrization) { build(:withdraw_parametrization, maximum_amount: amount) }

    it 'saves as multiple of 1e8' do
      expect(withdraw_parametrization[:maximum_amount]).to eq((amount.to_f * 1e8).to_i)
    end

    it 'return as decimal of 1e8' do
      expect(withdraw_parametrization.maximum_amount).to eq(amount)
    end
  end
